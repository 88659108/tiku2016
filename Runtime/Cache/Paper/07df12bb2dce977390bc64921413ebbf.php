...
<div id="wrip">
  <div class="answer-top white-block">
    ...
    <div class="type-list">
      <ul id="js-paper-navs" qcount="<?php echo ($upinfo["qnums"]); ?>">
      	<?php if(is_array($navs)): $i = 0; $__LIST__ = $navs;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$nav): $mod = ($i % 2 );++$i;?><li <?php if(($i) == "1"): ?>class="current"<?php endif; ?> key="<?php echo ($nav["key"]); ?>" usetime="<?php echo ($nav["usetime"]); ?>"><?php echo ($nav["title"]); ?> (<span class="dig js-nav-speed"><?php echo ((isset($nav["speed"]) && ($nav["speed"] !== ""))?($nav["speed"]):"0"); ?></span>/<span class="dig js-nav-nums"><?php echo ($nav["nums"]); ?></span>)</li><?php endforeach; endif; else: echo "" ;endif; ?>
      </ul>
    </div>
  </div>
  ...
  <div class="right fun-right white-block"><div class="time-block"><span class="img time-img"></span>
  <p class="blod js-paper-usetime" totaltime="<?php echo ($upinfo["totaltime"]); ?>" usetime="<?php echo ($upinfo["usetime"]); ?>"></p></div>
  <a class="default-btn anser-btn js-paper-time-pause" href="#!">暂停</a>
  <a class="default-btn anser-btn js-paper-time-end" href="#!">下次再做</a>
  <a class="default-btn green-btn anser-btn js-paper-submit" href="#!">交卷</a>
  <a class="default-btn anser-btn js-paper-answer-save" href="#!">保存答案</a>
  </div>
</div>
	...
	<div class="subtopics-block white-block js-question" usetime="{{usetime}}" uanswer="{{uanswer}}" answer="{{answer}}" qid="{{info.qid}}"  {{if info.isajax}}ajax="1"{{/if}} {{if materialid}}sonindex="{{sonindex}}" materialid="{{materialid}}"{{/if}}>
        ...
        <div class="options-block">
          <ul class="js-options">
			...
			<span class="img ico-option"></span><span class="options left">{{opt.cs | trim}}.</span><span class="text left">{{if opt.img == 1}}<img src="/Uploads/xingce/option2/{{opt.text}}" />{{else}}{{opt.text}}{{/if}}</span><span class="time right js-use-time">{{if uanswer == trim(opt.cs)}}{{info.usetime}}{{/if}}</span></li>
			{{/each}}
